...
_Here it is used as a database to hold user auth details_
_I used redis mainly because data im working with is light weigth and a traditional relational database would be an overkill in my opinion_
_with redis i store user information such as username and password(which is hashed) in a redit hash , which is a data type in redis where values are stored under key value pairs like a python dict_
_also user tokens generated are stored under a key which is an id with a expiration time. Tokens genearated fro users are under expiration time . tokens are returned to users with the time they expire as well as and id which together with token is used for authentication to access endpoints with a security layer_
...
On failure status code 501 is returned indicating the resource could not returned. This will most definitley be a server 
...
On success the status code is 200, a json object with fields  `status` equal to "Ok", `name` equal to cat name  and `info` which is a description of the cat.
On failure a , in a situation when there is no match for requested cat, status code of 401 is returned. Also a json object is returned with fild `status` equal to "Bad"
and `Description` with and error message


* POST /api/cats/add

Adds a new cat with a description and returns the updated cat resource in database.
...
On failure  , when user account coul not be created and user not added , status code 401 is returned with with a json object with a description of the situation.
...
he request body must be a json object that defines fields `id` and `token` . Fields are required!
...
On failure , in situations where token is expired i, a status code 200 is still  returned with a json object with token status `Expired`
...
This endpoint requires authentication hence request body posted must be of form json that defines fields `id` and `token` equal to auth details returned to user on creation of account.
Fields are required!
...
## Examples
* Thw following requests are sent using curl
